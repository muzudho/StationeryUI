...
配置は layouts の cells に定義してください。セルに slots は書きません。GridLayout は binding の `cell: {"row": 0, "col": 1}`、DockLayout は `cell: {"dock": "top", "index": 2}` でモデルと対応させます。GridLayout は 0 始まり、DockLayout は方向ごとの 1 始まりです。旧形式の slot は互換読み込みだけを行います。詳しくは [セルキーとモデルの対応](style-settings/file-format.md#セルキーとモデルの対応)を参照してください。
...
cells は layouts 内に定義し、モデルとの対応は GridLayout の `row`・`col`、DockLayout の方向・`index` で指定します。１セル０～１モデルで、配置・余白を binding の別フィールドに重複して書きません。GridLayout は 0 始まり、DockLayout は方向ごとの 1 始まりで、セル位置は永続的な識別子ではありません。[セルキーとモデルの対応](style-settings/file-format.md#セルキーとモデルの対応)を参照してください。
