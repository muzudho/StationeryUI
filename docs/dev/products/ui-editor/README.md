...
モデルの階層や種類、未編集のレイアウト、未知の追加プロパティは保持する。旧 bindings は変換可能な場合 bindingsV2 に移行する。読み込み元への保存 API は設けず、従来の新規エクスポートを使う。
...
既存ファイルの検査では、開始ページからファイルを開き、サイズを編集し、ツリーの別レイアウトをクリックする。出力 JSON が bindingsV2 として有効で、読み込み元ファイルのハッシュが変わらないことも確認する。
