...
グリッドレイアウトのセルの中に、さらにグリッドレイアウトを置き、外側と内側をそれぞれ調整できる設計体験を目指します。
...
将来は、セルへの内側レイアウトの追加、親子の階層をたどる操作、外側と内側を確認しながらの調整、必要な対応付けの生成を GUI で行えるようにします。これにより、複雑な画面も人間へ調整を委譲しやすくします。現在の指定方法と制約は [入れ子のグリッドレイアウト](file-format.md#入れ子のグリッドレイアウト)を参照してください。
