...
## 2026-09-21 ツリースクロールバーのドラッグ

- 表示専用だったスクロールバーに、つまみのドラッグとトラックのページ移動を追加。描画・当たり判定を同じ計算に統一し、幅を画面上の 17px に変更。
- 通常倍率／150% で、領域外までドラッグして上下端へ移動、領域外で解放して停止、トラッククリックを描画スモークで検証。既存の開閉・ホイール・キーボード・ダイアログを含め 20 シナリオ成功。
