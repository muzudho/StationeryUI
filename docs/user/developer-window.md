...
ウィンドウの中身は Windows Forms のコントロールではなく、**文房具 UI のツリーとスプリットペーン**で描画する。
...
文房具 Id はコードの動作と結び付く固定の名前。デモではスタイルファイルの `models` に階層とともに宣言する。
AI コーディング時は `models` の宣言と C# の接続箇所を合わせて生成・保守する。人間が設定ファイルや開発者ウィンドウから Id を登録・編集する必要はない。
...
ゲームの UI オブジェクトを直接操作しない。ホストの起動方法は [開発者向けの組み込み手順](../dev/developer-window.md) を参照。
