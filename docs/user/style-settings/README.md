...
**別リポジトリーへ取り込むときは、[スタイル設定と F12 開発者ウィンドウを一式で取り込む](full-integration.md)から始めてください。** スタイル読み込みに加えて、F12 用の子プロセスホスト、起動引数の分岐、検査情報の更新、終了処理、配布確認までを導入範囲に含めます。ライブラリー参照だけでは F12 は接続されません。
...
AI コーディングで画面の機能を実装したあと、人間が実物を見ながら「この列を少し広く」「余白をもう少し小さく」と調整したくなることがあります。スタイル設定ファイルは、**その細かなレイアウト調整を人間に委譲できるようにする仕組み**です。

AI は C# の部品・動作・設定との接続を実装し、人間が変更できる配置をスタイル設定へ切り出します。人間は JSON やスタイル設定エディターで寸法・比率・余白を調整します。オートリロードを接続しておけば、対応する設定は C# を書き換えて再ビルドすることなく、実際の画面で確認できます。

くだけて言えば、AI から「ここからの見た目の微調整は、人間がやってね」と渡せるようにする考え方です。AI に何度も座標修正を依頼する代わりに、人間が好みに合わせて仕上げられます。そのため、AI は調整可能な項目と、まだ C# に残る制約を明示して引き継ぎます。
...
## 今後の目標 — 入れ子のレイアウトを GUI で設計する

フローティングレイアウトのセルの中に、さらにフローティングレイアウトを置き、外側と内側をそれぞれ調整できる設計体験を目指します。

配置エンジンは、コンテナーのモデルと bindings を使う入れ子の配置にすでに対応しています。一方、現在のエディターは主に layouts の定義を編集する段階で、セルから内側のレイアウトを追加し、必要な models / bindings まで GUI で構成する一連の操作は未実装です。

将来は、セルへの内側レイアウトの追加、親子の階層をたどる操作、外側と内側を確認しながらの調整、必要な対応付けの生成を GUI で行えるようにします。これにより、複雑な画面も人間へ調整を委譲しやすくします。現在の指定方法と制約は [入れ子のフローティングレイアウト](file-format.md#入れ子のフローティングレイアウト)を参照してください。
