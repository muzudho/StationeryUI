param([string]$Version = '0.5.0', [string]$RuntimeVersion = '8.0.31')
...
try {
    ...
    $project = [xml](Get-Content samples/StationeryUIEditor/StationeryUIEditor.csproj -Raw)
    if ($project.Project.PropertyGroup.Version -ne $Version) { throw 'Project version mismatch.' }
    $run = Join-Path $workspace ('artifacts/release/ui-editor-v' + $Version + '-' + (Get-Date -Format 'yyyyMMdd-HHmmss'))
    $publish = Join-Path $run 'StationeryUIEditor'
    if (Test-Path -LiteralPath $run) { throw 'Release output must be a new directory.' }
    New-Item -ItemType Directory -Path $run | Out-Null
    dotnet publish samples/StationeryUIEditor -c Release -r win-x64 --self-contained true "-p:RuntimeFrameworkVersion=$RuntimeVersion" -p:DebugType=None -p:DebugSymbols=false "-p:SourceRevisionId=$revision" -o $publish
    if ($LASTEXITCODE -ne 0) { throw 'Publish failed.' }
    $exe = Join-Path $publish 'StationeryUIEditor.exe'
    if ((Get-Item -LiteralPath $exe).VersionInfo.FileVersion -ne "$Version.0") { throw 'EXE version mismatch.' }
    $runtime = Get-Content (Join-Path $publish 'StationeryUIEditor.runtimeconfig.json') -Raw | ConvertFrom-Json
    ...
    foreach ($file in Get-ChildItem -LiteralPath $publish -Recurse -File) {
        ...
        if ($file.Extension -eq '.json' -and $file.Name -notin @('StationeryUIEditor.deps.json','StationeryUIEditor.runtimeconfig.json')) { throw 'Unexpected JSON in distribution.' }
        if ($file.Extension -eq '' -and $file.Name -ne 'LICENSE') { throw 'Unexpected extensionless file.' }
    }
    $zip = Join-Path $run "StationeryUIEditor-v$Version-win-x64.zip"
    ...
    foreach ($file in $files) {
        ...
        $copy = Join-Path (Join-Path $extract 'StationeryUIEditor') $relative
