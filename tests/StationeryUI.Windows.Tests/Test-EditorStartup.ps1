...
$directory = Join-Path $workspace ('artifacts/editor-startup/' + [guid]::NewGuid().ToString('N'))
...
$previousPages = $env:STATIONERYUI_EDITOR_TEST_PAGES
try {
    $env:STATIONERYUI_EDITOR_TEST_OUTPUT = $directory
    $env:STATIONERYUI_EDITOR_TEST_INPUT = ''
    $env:STATIONERYUI_EDITOR_TEST_PAGES = if ($DeletePage) { 'delete' } elseif ($Pages) { '1' } else { '' }
    # Exercise the copy selected by the demo launcher, including its runtime dependencies.
    $executable = Join-Path $workspace "samples/StationeryUI.Demo/bin/$Configuration/net8.0-windows/StationeryUIEditor.exe"
    $process = Start-Process -FilePath $executable -ArgumentList ('"' + $source + '"') -WindowStyle Hidden -PassThru
    if (!$process.WaitForExit(30000)) { throw 'Editor startup timed out.' }
    if ($process.ExitCode -ne 0) { throw "Editor startup failed. See $directory/error.txt" }
    if (!(Test-Path -LiteralPath (Join-Path $directory 'editor.png'))) { throw 'Screenshot missing.' }
    ...
    $env:STATIONERYUI_EDITOR_TEST_PAGES = $previousPages
}
